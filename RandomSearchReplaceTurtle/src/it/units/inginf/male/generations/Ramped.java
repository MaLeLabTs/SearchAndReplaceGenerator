...
public class Ramped implements Generation {
    ...
    public Ramped(int maxDepth, Context context) {
     ...
     /**
     * This method return a new primaryPopulation of the desired size. An half of the
     * primaryPopulation is generated through Growth algorithm, the other half by the
     * Full method
     * @param popSize the desired primaryPopulation size
     * @param populationID populationID id of subPopulation (in order to acquire NodeFactory)
    ...
    public List<Node> generate(int popSize, int populationID) {
        ...
        population.addAll(this.full.generate(popSizeGrow, populationID));
        population.addAll(this.grow.generate(popSizeFull, populationID));
