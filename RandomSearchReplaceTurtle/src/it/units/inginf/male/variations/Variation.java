...
public class Variation {
     ...
     * @param populationID the id of the population of this individual
     * @return two new individuals
     */
    public Pair<Node, Node> crossover(Node individualA, Node individualB, int populationID) {
        ...
        for (int tries = 0; tries < 20; tries++) {
            ...
            Node randomNodeA = pickRandomNode(newIndividualA, populationID);
            Node randomNodeB = pickRandomNode(newIndividualB, populationID);
            ...
            if (randomNodeA != null && randomNodeB != null) {
                ...
                if (checkMaxDepth(newIndividualA, 1, populationID)
                        && checkMaxDepth(newIndividualB, 1, populationID)
     ...
     */
    public Node mutate(Node individual, int populationID) {

        List<Node> newNodes = this.growth.generate(20, populationID);
        Node mutant = individual.cloneTree();


        for (Node newNode : newNodes) {

            Node randomNode = pickRandomNode(mutant, populationID);
            if (randomNode != null) {
                replaceNode(mutant, randomNode, newNode);
                if (checkMaxDepth(mutant, 1, populationID) && mutant.isValid()) {
                    break;
                }
            }
    ...
    private Node pickRandomNode(Node individual, int populationID) {
        EvolutionParameters param = context.getConfiguration().getSubConfiguration(populationID).getEvolutionParameters();
    ...
    private boolean checkMaxDepth(Node root, int depth, int populationID) {
        if (depth > context.getConfiguration().getSubConfiguration(populationID).getEvolutionParameters().getMaxDepthAfterCrossover()) {
            return false;
        }
        ...
        boolean ret = true;
        for (Node child : root.getChildrens()) {
            ret &= checkMaxDepth(child, depth + 1, populationID);
        }
