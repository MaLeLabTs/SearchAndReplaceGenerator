...
public class DefaultExecutionListener implements ExecutionListener,ExecutionListenerFactory {
    ...
    public void evolutionComplete(RunStrategy strategy, int generation, Node best, List<Ranking> population) {
        ...
        try {
            Node bestPop = population.get(0).getTree();
            StringBuilder abuilder = new StringBuilder();
            bestPop.describe(abuilder);
//            LOG.log(Level.INFO, "Found best individual of job {0} after {1} generations: {2}", new Object[]{jobId, generation, PostfixToInfix.postfixToInfix(abuilder.toString())});
            LOG.log(Level.INFO, "Found best individual of job {0} after {1} generations: {2}", new Object[]{jobId, generation, bestPop.toString()});
        } catch (Exception ex) {
