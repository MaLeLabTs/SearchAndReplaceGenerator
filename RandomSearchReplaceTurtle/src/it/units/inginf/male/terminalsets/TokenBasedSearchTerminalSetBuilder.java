 ...
 */
public class TokenBasedSearchTerminalSetBuilder implements TerminalSetBuilder {
    @Override
    public void setup(SubConfiguration configuration) {
        ...
        for (ExampleReplace example : dataset.getExamples()) {
            ...
            if (beforeAndAfter != null) {
                ...
                Set<String> beforeSubparts = Utils.subparts(before, 2, 4);
                Set<String> afterSubparts = Utils.subparts(after, 2, 4);
                for (String ngram : beforeSubparts) {
                    terminalSet.add(new Constant(Utils.escape(ngram)));
                }
                for (String ngram : afterSubparts) {
                    terminalSet.add(new Constant(Utils.escape(ngram)));
                }
